...
describe("sessionsForCard (group-by over events)", () => {
  ...
  test("two sessions that both worked a card each appear once, and a session that claimed two cards reports alsoWorked", () => {
    const { f, board } = fresh();
    ...
    f.releaseCard(scout, board.id, c1.num);
    // A second agent picks the card up and works it too: both are workers.
    f.claimCard(other, board.id, c1.num);
  ...
  test("declaredModel comes from the agent's own Runtime.model, independent of the observed harness_sessions.model", () => {
    ...
    f.claimCard({ ...scout, model: "sonnet" }, board.id, 1);
