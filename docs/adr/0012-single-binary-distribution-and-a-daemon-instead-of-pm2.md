...
- **From an installed binary** it spawns one detached child — itself, `serve`ing the embedded web app and the API on the same port (`:4747` by default, `FLOCK_PORT`/`--port` to move it, `127.0.0.1` unless `--host` says otherwise). (**Superseded by [ADR 0015](0015-bind-to-all-interfaces-by-default-and-advertise-only-reachable-urls.md):** the default bind host is `0.0.0.0`, not `127.0.0.1`; `--host`/`FLOCK_HOST` opt back to loopback.)
