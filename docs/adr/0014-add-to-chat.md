...
### Presentation: the quote stays text, and a mirror layer paints it

The first implementation stopped at inserting the markdown, which is what the composer then showed: `> **robrichardson** said:` over `> ive me a l` over the caret. Card #6 came from Rob seeing exactly that — "can we give it a visual treatment like how nib does?"

nib does *not* stage the quote as a chip or a block above the field: `PendingQuote` is only the hand-off, and `insertText` puts the quote into the `NSTextView` as ordinary text (design D6, and the strip-of-chips shape is listed there as explicitly out of scope). The treatment is `applyHighlighting()`, which walks the draft `.byLines` and paints any line whose trimmed start is `>` in the secondary label colour, before the `/command` token pass, leaving the `>` markers themselves alone because they are what the engine receives. Quoted lines "sit back a step from your own words". Removing a quote is editing text; a second quote is more `>` lines.

That is also what Rob wants for flock — "I want it inside of the text area so I can add text in between multiple of them" — so the quote stays ordinary text in the textarea and the treatment is painted, not staged. A `<textarea>` cannot colour one of its lines, so:

- `draftHighlight.ts` (pure) turns the draft into one `DraftLine` per line: `quote` (nib's own test — trimmed, the line starts with `>`), `start`/`end` of a run of quoted lines, and the line split into `shown`/`hidden` spans. The hidden spans are the `> ` marker and the `**` around an author's name.
- `LineComposer` wraps the textarea in a `.composer-field` and, **only when the draft actually holds a quoted line**, renders `.composer-hl` — a mirror of the field's value — behind it, with the field's own glyphs set to `transparent` (and `caret-color` restated, since it would otherwise follow `color`). With nothing to paint there is no overlay and the field is exactly as it was.
- The mirror is glyph-for-glyph aligned with the real text, and that is the whole constraint on its CSS: **colour, background and inset shadows only**. A quoted line gets a `--bg-3` band, `--muted` ink and a 2px `--accent` left rule drawn with `inset` `box-shadow` (a border would move every glyph on the line); the band bleeds into the field's padding gutter with a negative margin and an equal padding, which cancel. A hidden span is hidden by `color: transparent` — the characters keep their columns, which is what turns `> **robrichardson** said:` into an indented `robrichardson said:` without moving anything. Font weight, size, letter-spacing and padding are never changed, and the mirror restates the field's type and padding wherever the field's own rules set them (the base rule, the desktop step, and the two collapsing `padding-block` rules).
- The field stacks above the mirror (`z-index`), so its translucent selection tint lands *over* the band and a drag-select reads normally. `::selection { color }` is deliberately not set: giving the selected run its real glyphs back would flash the raw markdown the treatment exists to hide.
- The mirror does not scroll itself; it is moved to the field's `scrollTop` on the field's `scroll` event and after every text change (growing past `max-height` scrolls the field without firing one).

Nothing rewrites the draft string, so the posted body, the draft store, `localStorage` persistence, autogrow, Enter-to-send, paste/drop and the insertion hand-off above are all untouched, and the message that reaches the channel is byte-identical to what this feature posted before the treatment existed. Editing, splitting and interleaving quotes with typed text all work because they are only ever text edits.

Not chosen: a `contenteditable` rewrite (quote atoms, a real document model). It would allow the `>` markers to actually disappear rather than merely go transparent, at the cost of reimplementing the field — undo, IME, paste, autogrow, Enter-to-send — and nib does not do it either.
...
- `blockquote` styling is a left rule on `--line` plus `--text`, so the contrast gate (`scripts/contrast.ts`) needs no new token pair. The composer's quote band is `--muted` on `--bg-3`, a pair the gate already checks, and it dims with a colour rather than an `opacity` — which the gate's dimmer scan would otherwise flag.
- The mirror is a second place the field's type and padding are stated. A future change to the composer's padding or font size that misses `.composer-hl` shows up immediately as a picture sitting off its text, which is the cheapest failure mode available short of measuring the field in JS every frame.
- Every composer gets the treatment, not just the channel's: a `> ` line typed into a card comment by hand is banded too. That is the same text meaning the same thing, and `markdown.tsx` renders it as a blockquote once sent.
