...
| `card.answered` | The human answered an ask | The card goes back to `doing` if it still has an assignee, `todo` if it does not — a subagent that followed the contract released it, so expect `todo`. Delegate a fresh agent to it; the answer is a comment on the card. The answer may be an emoji instead of words — a human double-tapped the ask panel to react rather than typing (`viaReaction: true` on the event) — so read it the same as a message reaction: 👍 approve, 👎 reject, anything else is tone. |
| `comment.posted` | The human commented on a card | Instruction or context for that card. The listener line ends with the comment's ref (`<card>.<n>`) — react `flock react <ref> 👍 --as conductor` on receipt, as the ack, then act. Follow with a `flock comment` only when there is something to relay or fold into the next delegation; a pure acknowledgement gets no reply beyond the reaction. A card comment carries images the same way a channel message does, so a line with `[attachments: ...]` is handled the same way: `flock attachment get <id> --out /tmp/flock-<id>.<ext>` for each, Read the file, and transcribe it for any subagent. |
...
React 👍 on receipt of every human message and card comment, as the ack, then act. Reply where the
human is looking only when there is something to say: a channel message gets a `flock say`; a card
comment gets a `flock comment`. A pure acknowledgement gets no reply beyond the reaction (see the
table above).
